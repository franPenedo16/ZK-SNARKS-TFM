...
REPS=35         
WARMUPS=5
CMD="snarkjs groth16 prove ../circuit_0001.zkey ../witness.wtns ../proof.json ../public.json"
OUTPUT="memory_rss_usage.json"
...
for run in $(seq 1 $REPS); do
    ...
    while kill -0 "$pid" 2>/dev/null; do
        ...
        rss_kib=$(grep Rss /proc/$pid/smaps 2>/dev/null | awk '{sum += $2} END {print sum}')
        rss_kib=${rss_kib:-0}
        if [ "$rss_kib" -eq 0 ]; then
            break
        fi

        rss_perc=$(awk "BEGIN { printf \"%.2f\", ($rss_kib/$total_mem_kib)*100 }")
        now_ts=$(date +%s.%N)
        ...
        if [ $save -eq 1 ]; then
            ...
            echo "      {\"timestamp\": $rel_ts, \"kib\": $rss_kib, \"percent\": $rss_perc}" >> "$OUTPUT"
        fi
